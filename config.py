...
parser.add_argument('--voc_type', type=str, default='ALLCASES_SYMBOLS',
                    choices=['LOWERCASE', 'ALLCASES', 'ALLCASES_SYMBOLS', 'CHINESE_7715'])
...
parser.add_argument('--weight_decay', type=float,
                    default=0.0)  # the model maybe under-fitting, 0.0 gives much better results.
...
parser.add_argument('--test_save', type=str, default='test_result.txt', metavar='PATH')
...
parser.add_argument('--test_freq', type=int, default=13336)
# misc
working_dir = osp.dirname(osp.dirname(osp.abspath(__file__)))
parser.add_argument('--logs_dir', type=str, metavar='PATH',
                    default=osp.join(working_dir, 'logs'), help="save the checkpoint")
parser.add_argument('--real_logs_dir', type=str, metavar='PATH',
                    default='/media/mkyang/research/recognition/selfattention_rec', help="创建软链接的地址")
parser.add_argument('--debug', action='store_true',
                    help="if debugging, some steps will be passed.")
# 暂未弄清vis_dir的作用，自己补的为输出test的结果
